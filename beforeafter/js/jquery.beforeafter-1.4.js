...
(function($){
	$.fn.extend({ 
		...
		var randID =  Math.round(Math.random()*100000000);
			...
			$(obj).prepend('<div id="dragwrapper'+randID+'"><div id="drag'+randID+'"><img width="8" height="56" alt="handle" src="/beforeafter/js/handle.gif" id="handle" /></div></div>'); // Create drag handle
			$('#dragwrapper'+randID).css({'opacity':.25,'position':'absolute','padding':'0','left':(imgWidth*o.introPosition)-($('#handle').width()/2)+'px','z-index':'20'}).width($('#handle').width()).height(imgHeight);
			...
			$('#drag'+randID).width(2).height(imgHeight).css({'background': 'green','position':'absolute','left':'3px'});	// Set drag handle CSS properties
			$('#beforeimage'+randID).css({'position':'absolute','top':'0px','left':'0px'});
			$('#afterimage'+randID).css({'position':'absolute','top':'0px','right':'0px'});
			$('#handle').css({'z-index':'100','position':'relative','cursor':o.cursor,'top':(imgHeight/2)-($('#handle').height()/2)+'px','left':'-3px'})

			$(obj).append('<img src="/beforeafter/js/lt-small.png" width="7" height="15" id="lt-arrow'+randID+'"><img src="/beforeafter/js/rt-small.png" width="7" height="15" id="rt-arrow'+randID+'">');
